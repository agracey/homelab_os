...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
    ...
    for path in paths {
        ...
        let filenameOS = path.unwrap().file_name();
        let filename = filenameOS.to_str().unwrap();
        let matches = filename.starts_with("epinio") && filename.ends_with("0");
        if matches {
            println!("tmpfile name: {:?}", filename)
        }
    }
